(ns godotclj.connect
  ...
  is stored in `signal-registry`. Then Godot signal is connected to the helper
  node's  method `signal-handler-method-name` and binds `::signal-entry` id
  with it.
...
(defn- disconnect-handler
  ...
  (let [instance-id (.getInstanceId node)]
    (if (signal-registered? registry instance-id signal-name)
      ...
      (do (util/warn!
           (format
            "No function to disconnect on object %s with signal %s!\n"
            node
            signal-name))
          registry))))
